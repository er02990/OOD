...
public class RobotCreation extends JFrame {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (ps < 5 || ps >= h - 5 || pe < 5 || pe >= w - 5) {
						errMsg.setText("Invalid input.\nPosition is out of bounds.\nUnits South: 5-715 inclusive\nUnits East: 5-1175 inclusive");
					}
					else if (inBuilding(ps, pe)) {
						errMsg.setText("You cannot create a robot\ninside the building.\nThe building is from 150-300 units\nE and S, exclusive.");
					}
					else {
						RobotFactory rf = RobotFactory.getFactory();
						dr = robotMake(rf, Power, ps, pe, name);
						
						boolean taken = isTaken(robots, ps, pe);
						boolean badName = hasBadName(robots, dr);
	...
	private boolean inBuilding(int ps, int pe) {
		return ps > 150 && ps < 300 && pe > 150 && pe < 300;
	}
	
	private DeliverRobot robotMake(RobotFactory rf, JRadioButton Power, int ps, int pe, String name) {
		if (Power.isSelected()) {
			return rf.makeRobot(ps, pe, true, name);
			
		}
		else {
			return rf.makeRobot(ps, pe, false, name);
		}
	}
	
	private boolean isTaken(ArrayList<DeliverRobot> robots, int ps, int pe) {
	...
	private boolean hasBadName(ArrayList<DeliverRobot> robots, DeliverRobot dr) {
		for(DeliverRobot var : robots) {
