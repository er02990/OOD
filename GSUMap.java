...
//this class is the main window that is used for the application
//it displays a map and is used to create, delete, and move robots that are displayed on it
//many attributes are marked with default visibility so that they can be conveniently accessed by other classes, but only in the same package
	private JFrame f;
	...
	public void createUI() {
		...
		//comboBox that will hold the names of the robots in the robots ArrayList
		...
		//create robot will launch a robotCreation window which is used to add robots to the map
		JButton btnCreateRobot = new JButton("CREATE ROBOT");
		btnCreateRobot.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				RobotCreation rc = new RobotCreation(720, 1180, robots, entireMap, combox);
				rc.setVisible(true);
				repaint();					
			}
		});
		...
		//listener will delete the current robot from the robots ArrayList and from combobox
		...
		//this button toggles the power of the selected tobot, if there is one selected
